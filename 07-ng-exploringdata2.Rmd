...
- `map_dbl`: Like `map`, but returns a numeric vector instead of a list.

Here is the syntax for `map`:

```{r cal, eval = FALSE}
## Generic code
map(.x = [list, vector, or dataframe], .f = [function], ...)
```
...
For this part of the exercise, you'll use a dataset on weather, air pollution, and mortality counts in Chicago, IL. This dataset is called `chicagoNMMAPS` and is part of the `dlnm` package. Change the name of the dataframe to  `chic` (this object name is shorter and will be easier to work with). Check out the data a bit to see what variables you have, and then perform the following tasks:
...
- Gather the variables `cvd`, `resp`, `temp`, `dptp`, `rhum`, `pm10`, and `o3` into a longer dataframe. Given the gathered columns the names `var` and `val`. How many row does the new dataframe contain?
...
- Based on this regression, does there seem to be a relationship between death and dewpoint temperature in Chicago? (Hint: Try using `glance` and `tidy` from the `broom` package on the model object to get more information about the model you fit.) What is the coefficient for dew point temperature? What is the p-value for the coefficient for dew point temperature?
...
lm_estimates <- lm_models %>%
                mutate(tidy_lm = mod %>% map(tidy),
                       estimates_lm = tidy_lm %>% map("estimate")) %>%
                select(var, estimates_lm) %>%
                filter(var == "dptp")
```
...
details_glance <- lm_models %>%
                  mutate(glance_lm = mod %>% map(glance),
                         p_value = glance_lm %>% map_dbl("p.value")) %>%
                  select(var, p_value) %>%
                  filter(var == "dptp")
```
...
```{r warning = FALSE, message = FALSE, fig.width = 5, fig.height = 4}
  details_augment$augment_lm[[which(details_augment$var == "dptp")]]  %>% 
  ggplot(aes(x = val, y = death)) +
  geom_point(size = 0.8, alpha = 0.5, col = "gray") +
  geom_line(aes(x = val, y = .fitted), color = "red", size = 2) +
  theme_classic() +
  xlab("Dew Point Temperature oC")
```
...
compare_models <- chic_long %>%
                  ...
                  mutate(mod = map(data, mod_glm),
                        ...
                        estimate_glm = tidy_lm %>% map("estimate")) %>%
                  select(var, estimate_glm) %>%
                  filter(var == "dptp") %>%
                  inner_join(lm_estimates, by = "var")
```
