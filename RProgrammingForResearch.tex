...
##  [5] "icd-10.xls"              "ICU_Data_Code_Sheet.pdf"
##  [7] "icu.sas7bdat"            "ld_genetics.txt"        
##  [9] "measles_data"            "mexico_deaths.csv"      
## [11] "mexico_exposure.csv"
\end{verbatim}
...
\begin{verbatim}
##         time          X         Y         Z
## 1 2009-01-01 -0.1686046 -1.042160  2.168506
## 2 2009-01-02 -0.2677525  1.372194  8.958164
## 3 2009-01-03 -1.0904974  1.345752 -6.631446
\end{verbatim}
...
\begin{verbatim}
##         time stock       price
## 1 2009-01-01     X -0.16860460
## 2 2009-01-02     X -0.26775253
## 3 2009-01-03     X -1.09049745
## 4 2009-01-04     X -1.44430967
## 5 2009-01-05     X -0.05977518
\end{verbatim}
...
\begin{verbatim}
##         time           X          Y         Z
## 1 2009-01-01 -0.16860460 -1.0421599  2.168506
## 2 2009-01-02 -0.26775253  1.3721944  8.958164
## 3 2009-01-03 -1.09049745  1.3457521 -6.631446
## 4 2009-01-04 -1.44430967  1.3198569  6.350252
## 5 2009-01-05 -0.05977518 -0.5423924  1.698341
\end{verbatim}
...
##   stock     N       mean        sd
##   <chr> <int>      <dbl>     <dbl>
## 1     X    10 -0.2921759 0.8007196
## 2     Y    10 -0.2387541 1.4425317
## 3     Z    10 -0.6488674 5.5782291
\end{verbatim}
...
Statistic & X & Y & Z\\
\hline
Mean (Std.dev.) & -0.29 (0.8) & -0.24 (1.44) & -0.65 (5.58)\\
\hline
N & 10 & 10 & 10\\
\hline
\end{tabular}
...
function. Here is an example of a very basic function. This function
takes a number as input and adds 1 to that number.
...
of pre-existing functions). You then include options (including any
defaults) and the code to run:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{## Note: this code will not run}
\NormalTok{[function name] <-}\StringTok{ }\NormalTok{function([any options and defaults])\{}
        \NormalTok{[code to run]}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}

Example: You want to take a log relative rate estimate determined per
minute and convert it to a relative rate per 90 minutes.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rr_per90 <-}\StringTok{ }\NormalTok{function(log_rr)\{}
        \NormalTok{out <-}\StringTok{ }\KeywordTok{exp}\NormalTok{(log_rr *}\StringTok{ }\DecValTok{90}\NormalTok{)}
        \KeywordTok{return}\NormalTok{(out)}
\NormalTok{\}}
\KeywordTok{rr_per90}\NormalTok{(pos_est$est[}\DecValTok{1}\NormalTok{])}
\end{Highlighting}
\end{Shaded}
...
Example: You want to take a vector of values for a log relative rate
estimate and its standard error and convert it to a pretty format of
relative rate and confidence intervals per 90 minute increase in playing
time:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pretty_rr90 <-}\StringTok{ }\NormalTok{function(vec)\{}
        \NormalTok{ests <-}\StringTok{ }\NormalTok{vec[}\DecValTok{1}\NormalTok{] +}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, -}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{) *}\StringTok{ }\FloatTok{1.96} \NormalTok{*}\StringTok{ }\NormalTok{vec[}\DecValTok{2}\NormalTok{]}
        \NormalTok{ests <-}\StringTok{ }\KeywordTok{round}\NormalTok{(}\KeywordTok{exp}\NormalTok{(}\DecValTok{90} \NormalTok{*}\StringTok{ }\NormalTok{ests), }\DecValTok{2}\NormalTok{)}
        \NormalTok{out <-}\StringTok{ }\KeywordTok{paste0}\NormalTok{(ests[}\DecValTok{1}\NormalTok{], }\StringTok{" ("}\NormalTok{, ests[}\DecValTok{2}\NormalTok{], }\StringTok{", "}\NormalTok{, ests[}\DecValTok{3}\NormalTok{], }\StringTok{")"}\NormalTok{)}
        \KeywordTok{return}\NormalTok{(out)}
\NormalTok{\}}
\KeywordTok{pretty_rr90}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\FloatTok{0.0031}\NormalTok{, }\FloatTok{0.00017}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
You can test out functions by walking through them, just like you can
with loops.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{vec <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{0.0031}\NormalTok{, }\FloatTok{0.00017}\NormalTok{)}
\NormalTok{(ests <-}\StringTok{ }\NormalTok{vec[}\DecValTok{1}\NormalTok{] +}\StringTok{ }\KeywordTok{c}\NormalTok{(}\DecValTok{0}\NormalTok{, -}\DecValTok{1}\NormalTok{, }\DecValTok{1}\NormalTok{) *}\StringTok{ }\FloatTok{1.96} \NormalTok{*}\StringTok{ }\NormalTok{vec[}\DecValTok{2}\NormalTok{])}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## [1] 0.0031000 0.0027668 0.0034332
\end{verbatim}
...
You can use \texttt{apply} to apply a function you've created to many
rows at a time. For example, you have log relative risk estimates and
standard errors for each position in \texttt{pos\_est}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pos_est[ , }\KeywordTok{c}\NormalTok{(}\StringTok{"est"}\NormalTok{, }\StringTok{"se"}\NormalTok{)]}
\end{Highlighting}
\end{Shaded}
...
## 4 0.005299009 7.146148e-05
\end{verbatim}

You can apply \texttt{pretty\_rr90} to each row (\texttt{MARGIN\ =\ 1})
of this part of the dataframe to get pretty estimates for each position:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{apply}\NormalTok{(pos_est[ , }\KeywordTok{c}\NormalTok{(}\StringTok{"est"}\NormalTok{, }\StringTok{"se"}\NormalTok{)], }\DataTypeTok{MARGIN =} \DecValTok{1}\NormalTok{, }\DataTypeTok{FUN =} \NormalTok{pretty_rr90)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
## [1] "1.32 (1.28, 1.36)" "1.52 (1.5, 1.53)"  "1.53 (1.52, 1.54)"
## [4] "1.61 (1.59, 1.63)"
\end{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{out_tab <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\StringTok{"Position"} \NormalTok{=}\StringTok{ }\NormalTok{pos_est$position,}
                      \StringTok{"Effect Estimate"} \NormalTok{=}\StringTok{ }\KeywordTok{apply}\NormalTok{(pos_est[ , }\KeywordTok{c}\NormalTok{(}\StringTok{"est"}\NormalTok{, }\StringTok{"se"}\NormalTok{)],}
                                                \DataTypeTok{MARGIN =} \DecValTok{1}\NormalTok{,}
                                                \DataTypeTok{FUN =} \NormalTok{pretty_rr90))}
\NormalTok{out_tab}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##     Position   Effect.Estimate
## 1 Goalkeeper 1.32 (1.28, 1.36)
## 2   Defender  1.52 (1.5, 1.53)
## 3 Midfielder 1.53 (1.52, 1.54)
## 4    Forward 1.61 (1.59, 1.63)
\end{verbatim}

And use \texttt{knitr}'s \texttt{kable()} function to print it:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{kable}\NormalTok{(out_tab, }\DataTypeTok{format =} \StringTok{"markdown"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{longtable}[c]{@{}ll@{}}
\toprule
Position & Effect.Estimate\tabularnewline
\midrule
\endhead
Goalkeeper & 1.32 (1.28, 1.36)\tabularnewline
Defender & 1.52 (1.5, 1.53)\tabularnewline
Midfielder & 1.53 (1.52, 1.54)\tabularnewline
Forward & 1.61 (1.59, 1.63)\tabularnewline
\bottomrule
\end{longtable}

\section{\texorpdfstring{\texttt{apply} family of
functions}{apply family of functions}}\label{apply-family-of-functions}
...
A matrix is like a dataframe, but all the values in all columns must be
...
A list has different elements, just like a dataframe has different
columns. However, the different elements of a list can have different
lengths (unlike the columns of a dataframe). The different elements can
also have different classes.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bar <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{a =} \KeywordTok{c}\NormalTok{(}\StringTok{"a"}\NormalTok{, }\StringTok{"b"}\NormalTok{, }\StringTok{"c"}\NormalTok{),}
            \DataTypeTok{b =} \DecValTok{1}\NormalTok{:}\DecValTok{5}\NormalTok{, }
            \DataTypeTok{c =} \KeywordTok{c}\NormalTok{(}\OtherTok{TRUE}\NormalTok{, }\OtherTok{FALSE}\NormalTok{))}
\NormalTok{bar}
\end{Highlighting}
\end{Shaded}
...
this either with numbers (which element in the list?) or with names or
the \texttt{\$} operator:
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bar[[}\StringTok{"b"}\NormalTok{]]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{head}\NormalTok{(}\KeywordTok{names}\NormalTok{(my_mod))}
\end{Highlighting}
\end{Shaded}
...
\begin{verbatim}
## (Intercept)     c(1:10) 
##  0.72612480 -0.05599286
\end{verbatim}

There is also a whole family of \texttt{apply} functions, as part of R's
based package. These include:
...
Convention for \texttt{apply}:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{apply}\NormalTok{([matrix], }\DataTypeTok{MARGIN =} \NormalTok{[}\KeywordTok{margin} \NormalTok{(}\DecValTok{1}\NormalTok{:}\StringTok{ }\NormalTok{rows, }\DecValTok{2}\NormalTok{:}\StringTok{ }\NormalTok{columns)], }\DataTypeTok{FUN =}  \NormalTok{[function])}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{head}\NormalTok{(}\KeywordTok{apply}\NormalTok{(ex, }\DataTypeTok{MARGIN =} \DecValTok{1}\NormalTok{, sum))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##     Abdoun        Abe     Abidal Abou Diaby  Aboubakar      Abreu 
##          6        115         97        117         18         15
\end{verbatim}

\texttt{lapply()} will apply a function across a list. The different
elements of the list do not have to be the same length (unlike a
dataframe, where the columns all have to have the same length).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{(ex <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{a =} \KeywordTok{c}\NormalTok{(}\DecValTok{1}\NormalTok{:}\DecValTok{5}\NormalTok{), }\DataTypeTok{b =} \KeywordTok{rnorm}\NormalTok{(}\DecValTok{3}\NormalTok{), }\DataTypeTok{c =} \KeywordTok{c}\NormalTok{(}\DecValTok{5}\NormalTok{, }\DecValTok{3}\NormalTok{, }\DecValTok{3}\NormalTok{, }\DecValTok{2}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}
...
## 
## $b
## [1] -0.1404420  2.5018976 -0.7697629
## 
## $c
## [1] 5 3 3 2
\end{verbatim}
...
\texttt{sapply()} also applies a function over a list, but it returns a
vector rather than a list:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{sapply}\NormalTok{(ex, }\DataTypeTok{FUN =} \NormalTok{mean)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##         a         b         c 
## 3.0000000 0.5305642 3.2500000
\end{verbatim}
...
code in R. It is definitely faster than doing the same things with
loops. However, unless you are working with large datasets, you may not
notice a difference.

I would recommend using whichever method makes the most sense to you
until you run into an analysis that takes a noticable amount of time to
...
\item
  ...
  code for cleaning the data and some code for plotting. I will add one
  example answer after class, but I'd like you to try to figure it out
  yourselves first.
\end{itemize}

\begin{center}\includegraphics{RProgrammingForResearch_files/figure-latex/unnamed-chunk-388-1} \end{center}
...
\begin{center}\includegraphics{RProgrammingForResearch_files/figure-latex/unnamed-chunk-392-1} \end{center}
...
\tightlist
\item
  Adapt the code that you wrote for the loop and write a function that
  will create a state-specific plot when you call it. This function
  should have inputs of \texttt{datafr} and \texttt{which\_state}, where
  \texttt{datafr} is the dataframe with the full dataset
  (\texttt{accident} in this case) and \texttt{which\_state} is the name
  of the state for which you'd like to create the plot. Name your
  functions \texttt{make\_state\_plot}. Here are a few examples of
...
\begin{center}\includegraphics{RProgrammingForResearch_files/figure-latex/unnamed-chunk-395-1} \end{center}
...
\begin{center}\includegraphics{RProgrammingForResearch_files/figure-latex/unnamed-chunk-395-2} \end{center}
...
\begin{center}\includegraphics{RProgrammingForResearch_files/figure-latex/unnamed-chunk-395-3} \end{center}
